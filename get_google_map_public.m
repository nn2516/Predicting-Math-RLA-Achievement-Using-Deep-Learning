function [XX YY M Mcolor] = get_google_map_public(lat, lon, varargin)

%%
% This code had modified slightly from get_google_map by 2016, Val Schmidt
%
...
% GET THE IMAGE
options = weboptions('Timeout',1800); % set for very long time to account for time spent saving? Or because my internet is glitchy?
[M Mcolor] = webread(url,options);
M = cast(M,'double');
